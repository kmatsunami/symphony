defmodule SymphonyElixir.CoreTest do
  ...
  test "current WORKFLOW.md file is valid and complete" do
    ...
    assert Map.get(tracker, "repository") == "kmatsunami/symphony"
    assert Map.get(tracker, "state_label_prefix") == "status:"
    assert is_list(Map.get(tracker, "active_states"))
    assert is_list(Map.get(tracker, "terminal_states"))

    hooks = Map.get(config, "hooks", %{})
    assert is_map(hooks)
    assert Map.get(hooks, "after_create") =~ "git clone --depth 1 https://github.com/kmatsunami/symphony ."
  ...
  defp assert_due_in_range(due_at_ms, min_remaining_ms, max_remaining_ms) do
    ...
    assert remaining_ms >= max(min_remaining_ms - @due_time_jitter_ms, 0)
